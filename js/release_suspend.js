...
for(j = 0; j < 4; j += 1){s = l[6 + j * 3].split("<");s = s[0].substring(0, s[0].length - 9);if(s == a){$('#rdo' + i + j).attr('checked', 'checked');break;};};}else{c += 1;
};};alert("自动答题结束，有" + c + "道题没找到…… \n请您检查一遍后提交（如果发现题库有错误请告诉我；如果脚本太给力还是等二十分钟再提交，答题太快有可能被封号呢）。 \n欢迎您加入我们的讨论群一起讨论完善题库！ \n我们的QQ群号：204524182");
}catch(e){alert("发生错误，脚本将终止。 \n若您在答题页面运行的这个脚本，请截图并发给我。 \n错误信息： \n" + e);};
function b(q){switch(q){
//-----题库开始-----
