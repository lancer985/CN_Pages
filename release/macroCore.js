...
function k()
{
  alert("Bilibili自动答题脚本 V3.2.1（2014.06.21更新） \n" + v + " \n有可能需要很长时间，若提示脚本或网页长时间无响应请选择等待。" + t + "\n\n按确定开始执行。")
  var c = 0;
  var d = 0;
  ...
  try
  {
    for(i = 0; i < 100; i += 1)
    {
      ...
      if(a != "hyErrNotFound")
      {
        for(j = 0; j < 5; j += 1)
        {
          if(j == 4)
          {
            alert("题库错误，若您使用的是" + r + "或更新的版本的浏览器：请截图，然后找到这个问题并将问题和其答案都复制发给我。如果您知道如何查看网页源码，请找到该题和其答案相对应的源码并复制发给我（建议使用QQ群的临时会话，否则有可能出现特殊字符失效等问题）。 \n\n问题：\"" + n + "\" \n返回的答案：\"" + a + "\" \n最后一次判定调用：\"" + s + "\" \n缓存：\"" + l + "\"");
            if(f)
            {
              z(i, 0);
            };
    ...
    alert("自动答题结束，有" + c + "道题没找到，有" + d + "道题返回无效（题库错误）…… \n请您检查一遍并填写未找到的问题后提交（如果发现题库有错误请告诉我；如果脚本太给力还是等二十分钟再提交，答题太快有可能被封号呢）。" + t);
  }catch(e){
